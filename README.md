...
| **Deribit** | Full site + changelog | Crawls API reference; JSON-RPC, FIX and Starbase changelogs plus announcements (last few months) |
...
Some monitors have additional options (e.g., `--max-pages` for Bybit, `--months`, `--no-docs`, `--no-changelogs` and `--no-announcements` for Deribit).
